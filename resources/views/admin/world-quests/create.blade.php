<x-admin-layout>
    <div class="space-y-6">
        <div class="flex justify-between items-center">
            <h2 class="text-2xl font-semibold text-gray-900">Add New World Quest</h2>
        ...
        <div class="bg-white shadow sm:rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                <form action="{{ route('admin.world-quests.store') }}" method="POST">
                    @csrf
                    <div class="space-y-6">
                        <div>
                            ...
                            <div class="mt-1">
                                <select name="region_id" id="region_id" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md @error('region_id') border-red-300 text-red-900 placeholder-red-300 focus:outline-none focus:ring-red-500 focus:border-red-500 @enderror">
                                    ...
                                    @foreach($regions as $region)
                                        <option value="{{ $region->id }}" {{ old('region_id', $selectedRegionId ?? '') == $region->id ? 'selected' : '' }}>
                                            {{ $region->name }}
                                        </option>
                                    @endforeach
                                </select>
